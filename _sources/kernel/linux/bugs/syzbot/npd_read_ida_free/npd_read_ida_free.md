...
这个漏洞是由于在`ida_free`函数中没有正确处理空bitmap的情况导致的。patch通过添加对`bitmap`指针的空检查来修复这个问题，并通过添加全面的测试用例来确保修复的健壮性。

地址：https://syzkaller.appspot.com/bug?extid=51baee846ddab52d5230

- 引入
  - [181a42edddf51d5d9697ecdf365d72ebeab5afb](https://web.git.kernel.org/pub/scm/linux/kernel/git/next/linux-next.git/commit/?id=181a42edddf51d5d9697ecdf365d72ebeab5afb0)
  - v6.6-rc6-1738-g181a42edddf5
- 修复
  - [af73483f4e8b6f5c68c9aa63257bdd929a9c194a](https://web.git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=af73483f4e8b6f5c68c9aa63257bdd929a9c194a)
  - v6.7-rc6-167-gaf73483f4e8b

## 漏洞原理

这个漏洞出现在Linux内核的IDA（Integer Descriptor Allocator）子系统中，具体是在`ida_free`函数处理空bitmap的情况时。IDA用于管理整数标识符的分配和释放，通常用于内核中需要动态分配唯一整数ID的场景。

在原来的代码中，`ida_free`函数在释放一个ID时，会检查该ID在bitmap中是否被标记为已分配。如果bitmap为空（即没有分配任何ID），那么`bitmap`指针可能是`NULL`。在这种情况下，原来的代码没有正确检查`bitmap`是否为`NULL`，就直接访问了`bitmap->bitmap`，这可能导致空指针解引用，从而引发内核崩溃。

具体来说，原来的代码在`ida_free`函数中有如下检查：
...
当`bitmap`为`NULL`时，访问`bitmap->bitmap`会导致崩溃。

## Patch修复方法

这个patch通过在检查位是否被设置之前，先检查`bitmap`指针是否为`NULL`，从而避免了空指针解引用的问题。修复后的代码如下：
...
这样，当`bitmap`为`NULL`时，直接跳过位检查，避免访问空指针。

此外，patch还添加了新的测试用例到`test_ida.c`文件中，以确保在各种情况下（如IDA为空、包含单个值、单个bitmap或树结构等）尝试释放未分配的ID时不会出现问题。这些测试用例帮助验证修复的有效性，并防止未来类似问题的回归。

## 漏洞复现参数


- kernel: linux-next 8c9660f65153
- syzkaller: 5b429f39
- [config](./config)
- [syz repro](./repro.syz)
- [c repro](./repro.c)
...
void execute_one(void)
{
  ...
  *(uint8_t*)0x20000101 = 0xe;
  *(uint8_t*)0x20000102 = 0xf;
  *(uint8_t*)0x20000103 = 0;
  *(uint16_t*)0x20000104 = 0x2061;
  *(uint8_t*)0x20000106 = 8;
  *(uint16_t*)0x20000107 = 0xc8;
  *(uint16_t*)0x20000109 = 3;
  *(uint32_t*)0x2000010b = 0x209;
  memcpy((void*)0x2000010f, "\x83\x88\xf1", 3);
  syz_emit_vhci(/*data=*/0x20000100, /*size=*/0x12);
  memcpy((void*)0x20000000,
         "\x04\x3e\x82\x1d\xfa\xfd\xc4\x42\x57\x70\xea\xea\x47\x45\x9f\x2d\xcf"
         "\x21",
         18);
  syz_emit_vhci(/*data=*/0x20000000, /*size=*/0x85);
}
int main(void)
{
